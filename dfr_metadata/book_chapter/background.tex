...
We consider a tool as passing if each recovered file only contains data from one file on the original disk.
 ...
 We consider a tool as passing if each file it recovers is given the same file extension as it did before it was deleted. We exclude false positives as they have no corresponding file to compare to, and only consider files which were carved starting from a valid header signature.
