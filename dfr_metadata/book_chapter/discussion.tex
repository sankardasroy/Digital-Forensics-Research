...
Magnet AXIOM performs about the same as Scalpel on FC-CR-01, consistantly carving over two thirds of the deleted files.
...
Based on our evaluation, no one tool consistantly outperforms the others.
...
Since these have only complete, contigous files, they are expected to be easy.
...
Some tools output a smaller number of files but generally only valid files, while other tools may get more ``hits'' but also output many false posititives which do not constitute a valid file.
...
One other observation is that Foremost and Magnet AXIOM do not recover any TIF files from any of the test cases, despite both supporting the format.
\TODO{Does Magnet AXIOM support it?}
...
%% All tools has a harder time with empty speace than adjacent files, especially PhotoRec
