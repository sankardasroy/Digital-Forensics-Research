...
In particular, \TODO{we designed 13 test images for each of FAT and NTFS}, and we claim that these cover most of the real-life scenarios.
...
For instance, a tool may employ a \emph{strong match policy} to identify a deleted file, which can help the tool avoid retrieving a \emph{bogus file} 
...
\TODO{came out in 2014, which evaluate an outdated version of these tools}.   

As we have tried to match DFR tools' behavior in many test scenarios with NIST CFTT expectations, we have also got an first-hand experience of applicability 
of the NIST CFTT guidelines in a practical setting. For instance, we observe that some of the expectations are too high whereas a few others are unreasonable 
and have room for improvement. We provide critique on applicability of the NIST CFTT guidelines.
