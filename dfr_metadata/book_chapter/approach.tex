...
Most tools recover files perfectly for FAT case 2, and all tools recover files perfectly for FAT case 1 and NTFS cases 1-3.
...
Note that in the two test cases where core feature 1 is not met, meaning no deleted file was detected, we cannot make a judgement for the other core features as they are all dependent on core feature 1.
...
This is most easily observed in FAT case 2; Autopsy, FTK, and TestDisk will skip over file B to recover all of file A, while Recuva and Magnet AXIOM will recover the first fragment of file A along with the active file B.
This is why Recuva and Magnet AXIOM fail core feature 4 for that case.
However, when a file is fragmented around another deleted file, like in FAT case 3, all tools take the first approach and erroneously return file B as part of file A.
...
In NTFS more information about a file's location remain after the file is deleted, so recovering fragmented files is trivial.
All tools we tested are able to handle fragmentation in NTFS with no issues.


\paragraph{Recovering Overwritten Files}
We observe that when a deleted file is overwritten by an active file, most tools still attempt to recover the overwritten part as though it was part of the original file, failing core feature 4.
The only tool to consistently meet core feature 4 in these cases is FTK Imager, which only recovers data from before the overwritten part.
TestDisk also does this for FAT case 5 only, otherwise behaving like the other tools.
...
For FAT cases 4 and 5, Magnet AXIOM recovers up to the end overwritten sections, but no further.
