...
\maketitle
\begin{abstract}
Digital forensic (DF) tools are used for post-mortem investigation of cyber-crimes and cyber-attacks. National Institute of Standards and Technology 
has set standards for DF tools. Compliance of the standards by the DF tools is critical, especially in judical proceedings: 
Usage of a forensic tool that does not follow the standards may cause evidence to be thrown out in a court case 
whereas incorrect results from a forensic tool can also lead improper prosecution of an innocent defendant. 
...
We also compare those tools' performance and publish the results, with the aim to help the user choose the right DFR tool. 
\end{abstract}
